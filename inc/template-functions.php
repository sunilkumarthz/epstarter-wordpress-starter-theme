	...
	function epstarter_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf(
					'<span class="cat-links"><i class="fa fa-thumbtack"></i> <span class="screen-reader-text">%1$s</span>%2$s</span>',
					__( 'Posted in ', 'epstarter' ),
					$categories_list
				); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf(
					'<span class="tags-links"><i class="fa fa-tags"></i> <span class="screen-reader-text">%1$s </span>%2$s</span>',
					__( 'Tags:', 'epstarter' ),
					$tags_list
				); // WPCS: XSS OK.
			}
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'epstarter' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'epstarter_posted_on' ) ) :
	...
	function epstarter_posted_on() {
		...
		printf(
			'<span class="posted-on"><i class="fa fa-clock"></i> <a href="%1$s" rel="bookmark">%2$s</a></span>',
